...
;; starts the process, no "startup" required
(def zk (create-zookeeper {:zookeeper-port 2182
                           :kafka-port 9999
                           :topic "test"}))
(.shutdown zk)
;; special "startup" required to start the process
(def broker (create-broker  {:zookeeper-port 2182
                           :kafka-port 9999
                           :topic "test"}))
(.startup broker)
(.shutdown broker)
